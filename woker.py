...
from ../utils import build_logger, StreamToLogger, pretty_print_semaphore
...
"""
@sgl.function装饰器定义SGLang函数
    -> 封装提示工程逻辑（如多轮对话、分支处理） -> 仅支持RuntimeEndpoint后端

    eg.
        sgl.gen(): 生成文本, 支持参数约束（如max_tokens, regex）。
        sgl.select():从选项中选择概率最高的结果（需配合choices_method）。
        sgl.image()：处理多模态输入（如图像）。
        sgl.fork()/sgl.join()：实现并行分支与合并
"""
@sgl.function
